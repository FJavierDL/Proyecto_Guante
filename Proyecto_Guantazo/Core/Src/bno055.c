...
//
//
//void bno055_setPage(uint8_t page) { bno055_writeData(BNO055_PAGE_ID, page, i2c_address); }
//
//bno055_opmode_t bno055_getOperationMode() {
//  bno055_opmode_t mode;
//  bno055_readData(BNO055_OPR_MODE, &mode, 1, i2c_address);
//  return mode;
//}
//
//void bno055_setOperationMode(bno055_opmode_t mode) {
//  bno055_writeData(BNO055_OPR_MODE, mode, i2c_address);
//  if (mode == BNO055_OPERATION_MODE_CONFIG) {
//    bno055_delay(19);
//  } else {
//    bno055_delay(7);
...
//  bno055_setPage(0);
//  uint8_t tmp = 0;
//  bno055_readData(BNO055_SYS_TRIGGER, &tmp, 1, i2c_address);
//  tmp |= (state == true) ? 0x80 : 0x0;
//  bno055_writeData(BNO055_SYS_TRIGGER, tmp, i2c_address);
//  bno055_delay(700);
//}
//
//void bno055_enableExternalCrystal() { bno055_setExternalCrystalUse(true); }
...
//
//void bno055_reset() {
//  bno055_writeData(BNO055_SYS_TRIGGER, 0x20, i2c_address);
//  bno055_delay(700);
//}
//
//int8_t bno055_getTemp() {
//  bno055_setPage(0);
//  uint8_t t;
//  bno055_readData(BNO055_TEMP, &t, 1, i2c_address);
//  return t;
//}
//
//void bno055_setup() {
...
//
//  uint8_t id = 0;
//  bno055_readData(BNO055_CHIP_ID, &id, 1, i2c_address);
//  if (id != BNO055_ID) {
//    printf("Can't find BNO055, id: 0x%02x. Please check your wiring.\r\n", id);
//  }
//  bno055_setPage(0);
//  bno055_writeData(BNO055_SYS_TRIGGER, 0x0, i2c_address);
//
//  // Select BNO055 config mode
//  bno055_setOperationModeConfig();
//  bno055_delay(10);
...
//  bno055_setPage(0);
//  uint8_t buffer[2];
//  bno055_readData(BNO055_SW_REV_ID_LSB, buffer, 2, i2c_address);
//  return (int16_t)((buffer[1] << 8) | buffer[0]);
//}
//
//uint8_t bno055_getBootloaderRevision() {
//  bno055_setPage(0);
//  uint8_t tmp;
//  bno055_readData(BNO055_BL_REV_ID, &tmp, 1, i2c_address);
//  return tmp;
//}
//
//uint8_t bno055_getSystemStatus() {
//  bno055_setPage(0);
//  uint8_t tmp;
//  bno055_readData(BNO055_SYS_STATUS, &tmp, 1, i2c_address);
//  return tmp;
//}
//
//bno055_self_test_result_t bno055_getSelfTestResult() {
...
//  bno055_self_test_result_t res = {
//      .mcuState = 0, .gyrState = 0, .magState = 0, .accState = 0};
//  bno055_readData(BNO055_ST_RESULT, &tmp, 1, i2c_address);
//  res.mcuState = (tmp >> 3) & 0x01;
//  res.gyrState = (tmp >> 2) & 0x01;
//  res.magState = (tmp >> 1) & 0x01;
//  res.accState = (tmp >> 0) & 0x01;
...
//  bno055_setPage(0);
//  uint8_t tmp;
//  bno055_readData(BNO055_SYS_ERR, &tmp, 1, i2c_address);
//  return tmp;
//}
//
//bno055_calibration_state_t bno055_getCalibrationState() {
...
//  bno055_calibration_state_t cal = {.sys = 0, .gyro = 0, .mag = 0, .accel = 0};
//  uint8_t calState = 0;
//  bno055_readData(BNO055_CALIB_STAT, &calState, 1, i2c_address);
//  cal.sys = (calState >> 6) & 0x03;
//  cal.gyro = (calState >> 4) & 0x03;
//  cal.accel = (calState >> 2) & 0x03;
//  cal.mag = calState & 0x03;
...
//  bno055_setPage(0);
//
//  bno055_readData(BNO055_ACC_OFFSET_X_LSB, buffer, 22, i2c_address);
//
//  // Assumes little endian processor
//  memcpy(&calData.offset.accel, buffer, 6);
//  memcpy(&calData.offset.mag, buffer + 6, 6);
...
//  for (uint8_t i=0; i < 22; i++) {
//    // TODO(oliv4945): create multibytes write
//    bno055_writeData(BNO055_ACC_OFFSET_X_LSB+i, buffer[i], i2c_address);
//  }
//
//  bno055_setOperationMode(operationMode);
//}
...
//
//  if (vec == BNO055_VECTOR_QUATERNION)
//    bno055_readData(vec, buffer, 8, i2c_address);
//  else
//    bno055_readData(vec, buffer, 6, i2c_address);
//
//  double scale = 1;
//
//  if (vec == BNO055_VECTOR_MAGNETOMETER) {
...
//  uint8_t axisRemap = (axis.z << 4) | (axis.y << 2) | (axis.x);
//  uint8_t axisMapSign = (axis.x_sign << 2) | (axis.y_sign << 1) | (axis.z_sign);
//  bno055_writeData(BNO055_AXIS_MAP_CONFIG, axisRemap, i2c_address);
//  bno055_writeData(BNO055_AXIS_MAP_SIGN, axisMapSign, i2c_address);
//}
